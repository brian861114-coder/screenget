...
- **互動性與細節增強**：
    ...
    - **設定頁面視覺強化**：將「語言設定」、「開機啟動」及「白名單標題」的字體放大至 `18px`，顯著提升閱讀舒適度。
    - 修正圖表標籤配色與字體，解決簡體中文或日文环境下可能的亂碼問題。
- **圖示相容性修復 (Icon Compatibility)**：
    - 導入 `.ico` 格式圖示：將原有的 `.png` 圖示轉換為多尺寸封裝的 `.ico` 檔案，徹底解決 Windows 工作列無法正常顯示自定義圖標的問題。
    - 優化載入時序：調整 `AppUserModelID` 的設定時機，確保在應用程式啟動的最早期即生效。
    - 統一圖示管理：整合 `main.py` 與 `ui/main_window.py` 的圖示載入 logic。
